...
public:
    ...
    void setReminder(string t, string msg) {
        time = t;
        message = msg;
    }
...
public:
    HealthReport(PersonInfo *pi, BMIMonitor *bm, string date)
        : person(pi), bmiMonitor(bm), reportDate(date) {}

    void generateDailyReport() {
    
        cout << "\nDaily Health Report  <" << reportDate <<">"<< endl;
        cout << "Name: " << person->getName() << "\nAge: " << person->getAge() << "\nFavorite Sport: " << person->getFavoriteSport() << endl;
...
int main() {
    Notification notification;
    FoodSuggestion foodSuggestion(&notification);
    OverallHealth overallHealth;
    ExerciseMotivation exerciseMotivation(&notification);
